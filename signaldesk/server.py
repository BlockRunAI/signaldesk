...
def serve(port=8787, env_path=None):
    env_path = Path(env_path) if env_path is not None else ROOT / ".env"
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if self.path not in ('/api/run','/api/settings'):return self.send(404,{'error':'Not found'})
            try:
            ...
            if self.path == '/api/settings':
                with lock:
                    if state['busy']:return self.send(409,{'error':'Wait for the current run before changing credentials'})
                    try:result=update_settings(options,env_path)
                    except ValueError as e:return self.send(400,{'error':str(e)})
                    except OSError:return self.send(500,{'error':'Could not save the local settings file; no changes applied'})
