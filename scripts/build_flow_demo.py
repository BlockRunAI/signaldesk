...
content=f'''<div id="root" data-composition-id="signaldesk-flow" data-width="1920" data-height="1080" data-duration="24">
<div id="flow-scene" class="clip" data-start="0" data-duration="24" data-track-index="1">
<div class="paper"></div><div class="brand"><b>s</b>signaldesk</div><div class="byline">BLOCKRUN + TYPESAFE JEV</div>
...
web=head+'<link rel="stylesheet" href="/flow.css"><script src="/vendor/gsap.min.js" defer></script><script src="/flow.js" defer></script><script src="/flow-player.js" defer></script></head><body class="player-body"><div class="player-viewport"><div class="player-scale">'+content+'</div></div><nav class="player-controls"><button id="play">Pause</button><button id="replay">Replay</button><label for="seek">Timeline</label><input id="seek" type="range" min="0" max="24" step="0.05" value="0"><a href="/?run='+esc(r['id'])+'&amp;filter=review&amp;draft=1">Open verified run ↗</a><small>Saved data · No paid API calls during playback</small></nav></body></html>'
...
(v/'index.html').write_text(head+'<script src="assets/gsap.min.js"></script><style>'+(ROOT/'web/flow.css').read_text()+'</style></head><body>'+content+'<script>'+(ROOT/'web/flow.js').read_text()+'</script></body></html>')
...
print('Built English replay page and 24-second composition from verified data.')
